...
public partial class Production_Bending : System.Web.UI.Page
{
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (btnEdit != null)
                {
                    ...
                    if (Dt.Rows.Count > 0)
                    {
                        ...
                        SqlDataAdapter Sdd = new SqlDataAdapter("Select * FROM tblUserRoleAuthorization where UserID = '" + id + "' AND PageName = 'Bending.aspx' AND PagesView = '1'", con);
                        Sdd.Fill(Dtt);
                        if (Dtt.Rows.Count > 0)
                        {
 ...
 else
 {
    ...
    protected void MainGridLoad_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                //        " WHERE Pd.Stage = 'Bending' AND Pd.ProjectCode='{0}'", ProjectCode.Text));
    ...
    {
        Response.Redirect("Bending.aspx");
    }
